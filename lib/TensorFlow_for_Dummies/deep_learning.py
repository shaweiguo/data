...
w3 = tf.Variable(tf.random_normal([hid_nodes, hid_nodes]))
...
b3 = tf.Variable(tf.random_normal([hid_nodes]))
...
# Compute loss
loss = tf.reduce_mean(
    tf.nn.softmax_cross_entropy_with_logits(logits=out_layer, labels=lbl_holder)
)
...
with tf.Session() as sess:
    ...
    print('Success rate: ', sess.run(success, feed_dict={
        img_holder: dataset.test.images,
